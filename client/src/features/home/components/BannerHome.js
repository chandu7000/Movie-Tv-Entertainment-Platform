...
const BannerHome = () => {
  ...
  if (!slideCount) return <div className='h-[68vh] min-h-[500px] w-full animate-pulse bg-neutral-900' />;
    ...
    <section className='relative w-full overflow-hidden'>
      <div
        ...
        style={{
          ...
          transition: transitionEnabled ? 'transform 900ms cubic-bezier(0.22, 1, 0.36, 1)' : 'none',
        }}
                ...
                <div className='mx-auto w-full max-w-[1600px] px-4 pb-16 sm:px-6 lg:px-10 lg:pb-24'>
                  <div className='max-w-3xl'>
                    <div className='mb-3 flex flex-wrap items-center gap-2'>
                      ...
                      {(data.release_date || data.first_air_date) ? <span className='text-sm font-semibold text-neutral-300'>{moment(data.release_date || data.first_air_date).format('YYYY')}</span> : null}
                      {isCurrent && runtime ? <span className='text-sm font-semibold text-neutral-300'>{runtime}</span> : null}
                    </div>
                    <h1 className='max-w-3xl text-4xl font-black tracking-tight text-white drop-shadow-2xl sm:text-5xl lg:text-7xl'>{data?.title || data?.name}</h1>
                    {isCurrent && detailsData?.genres?.length ? <div className='mt-4 flex flex-wrap gap-2'>{detailsData.genres.slice(0, 4).map((genre) => <GenreChip key={genre.id}>{genre.name}</GenreChip>)}</div> : null}
                    <p className='mt-4 line-clamp-3 max-w-2xl text-sm leading-6 text-neutral-200 sm:text-base lg:text-lg lg:leading-7'>{data.overview || 'Discover this featured title on CineVerse.'}</p>
                    <div className='mt-6 flex flex-wrap gap-3'>
                      <Button onClick={handleTrailer} className='px-5 py-2.5'><FaPlay className='text-sm' /> Watch Trailer</Button>
                      <Button variant='secondary' className='px-5 py-2.5' onClick={() => handleDetails(data)}>View Details</Button>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {slideCount > 1 ? (
        <div className='absolute bottom-5 left-1/2 z-10 flex max-w-[92vw] -translate-x-1/2 items-center gap-3 rounded-full border border-white/10 bg-black/35 px-3 py-2 backdrop-blur-md'>
          ...
          <div className='flex items-center gap-1' role='tablist' aria-label='Featured titles'>
            {bannerData.map((item, index) => (
              <button
                key={`${item.id}-hero-indicator`}
                type='button'
                onClick={() => goToSlide(index)}
                className={`h-1.5 rounded-full transition-all duration-300 ${currentImage === index ? 'w-6 bg-white' : 'w-3 bg-white/35 hover:bg-white/60'}`}
      ...
      {playVideo && currentData && user ? <VideoPlay data={currentData} close={() => setPlayVideo(false)} media_type={mediaType} /> : null}
    </section>
  );
};
